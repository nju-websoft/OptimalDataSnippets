        ...
        for max_size in [snippet_max_size]:
            for epoch in [10]:
                for lr in ['1e-5', '3e-5', '5e-5']:
                    with open(f'{model_path}/pool_data_{segment_method}_{max_size}_{alpha}_reranker_reranking{extra}_lr_{lr}_bs_2_epoch_{epoch}.tsv', 'w+') as fout:
                        for fold in range(0,5):  
                            print(f'acordar fold{fold} data_{segment_method}_{max_size}_{alpha}_{lr}_{epoch}')
                            split = (fold + 3) % 5 if mode == 'dev' else (fold + 4) % 5
                            with open(f'{rerank_path}/pool_{segment_method}_{max_size}_{alpha}_split_{split}.json', 'r') as fin:
                                test_json = json.load(fin)
                            reranker = FlagReranker(f'{model_path}/{segment_method}_{max_size}_{alpha}_reranker/fold{fold}/lr_{lr}_bs_2_epoch_{epoch}', use_fp16=True)    
