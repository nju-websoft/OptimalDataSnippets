    ...
    for query_dataset in query_dataset_list:
        with open(os.path.join(snippet_base, query_dataset, f'ours_{max_size}_{alpha}.json'), 'r') as f:
            ...
            if dataset_id not in query2dataset2text[query_id]:
                query2dataset2text[query_id][dataset_id] = ' '.join([triple['text'] for triple in triples[:max_size]])
...
def main():
    ...
    for split in range(5):
        ...
        output = os.path.join(output_path, f"{snippet_method}_{max_size}_{alpha}_split_{split}.jsonl")
    ...
    for fold in range(5):
        ...
        for split in [split1, split2, split3]:
            input_path = os.path.join(output_path, f"{snippet_method}_{max_size}_{alpha}_split_{split}.jsonl")
            with jsonlines.open(input_path, 'r') as reader:
                for result in reader:
                    results.append(result)
        output = os.path.join(output_path, f"{snippet_method}_{max_size}_{alpha}_split_{split1}{split2}{split3}.jsonl")
        with jsonlines.open(output, 'w') as writer:
